...
class ADMSController(http.Controller):
    ...
    def _process_attendance(self, serial, body):
        ...
        STATUS is the attendance state sent by the device.
        VERIFY is the verification method (finger, face, card, etc.).

        These two values must stay separate:
        - STATUS drives in/out interpretation in traditional modes
        - VERIFY populates the "Category" column in raw attendance logs

        Some devices send STATUS=255 when the terminal is not configured with
        an explicit attendance state. In that case we should preserve the raw
        punch as "Duplicate" / special status in the audit log instead of
        forcing it to "Check Out" for display.
        """
        device = self._find_device_by_serial(serial)
        if not device:
        ...
        for line in lines:
            ...
            try:
                ...
                with env.cr.savepoint(flush=False):
                    ...
                    # Preserve the raw device status for audit/display purposes.
                    # STATUS and VERIFY are different ADMS fields:
                    # - STATUS is attendance state (check-in/check-out/etc.)
                    # - VERIFY is biometric method (finger/face/card/etc.)
                    #
                    # Prior behavior incorrectly used STATUS for "Category" and
                    # forced any non-check-in STATUS to "Check Out", which made
                    # many normal punches appear as:
                    #   Punching Type = Check Out
                    #   Category      = Duplicate
                    #
                    # Keep the raw punch state in the ZK record, then derive a
                    # simplified in/out value only for the traditional modes.
                    raw_punch_type = ZkMachineAttendance._sanitize_punch_type(zk_status)
                    normalized_punch_type = (
                        0 if zk_status in (0, 3, 4)
                        else 1 if zk_status in (1, 2, 5)
                        else None
                    )
                    ...
                    zk_att = zk_attendance.create({
                        ...
                        'device_id': device.id,
                        'device_id_num': user_id,
                        'attendance_type': ZkMachineAttendance._sanitize_attendance_type(verify),
                    ...
                    if attendance_mode == 'auto':
                        ...
                        if normalized_punch_type is None:
                            _logger.warning(
                                "ADMS ATTLOG: Unsupported STATUS=%s for employee %s in "
                                "traditional_per_day mode. Raw punch stored but not processed.",
                                zk_status, employee.name)
                            odoo_action, att_record = (None, None)
                        else:
                            odoo_action, att_record = device._process_traditional_per_day_attendance(
                                employee, atten_time_str, normalized_punch_type, hr_attendance)
                    else:
                        if attendance_mode == 'traditional' and normalized_punch_type is None:
                            _logger.warning(
                                "ADMS ATTLOG: Unsupported STATUS=%s for employee %s in "
                                "traditional mode. Raw punch stored but not processed.",
                                zk_status, employee.name)
                            odoo_action, att_record = (None, None)
                        else:
