...
class HrPayslip(models.Model):
    ...
    def action_compute_sheet(self):
        ...
        for payslip in self:
            ...
            if not contract_ids and payslip.struct_id:
                # Allow the payslip structure to drive the computation when
                # the contract itself does not carry a payroll structure.
                contract_ids = payslip.contract_id.ids
            # Rebuild worked-day lines so attendance changes are reflected
            # when an existing payslip is computed again.
            ...
            # Rebuild the lines explicitly so Refresh Data cannot retain
            # stale attendance values in the one2many cache.
            ...
            payslip.invalidate_recordset(['worked_days_line_ids'])
    ...
    def action_refresh_data(self):
        """Refresh attendance, leave, input, and salary values on draft slips."""
        self.ensure_one()
        if self.state != 'draft':
            raise UserError(_('Only draft payslips can be refreshed.'))
        return self.action_compute_sheet()

    @api.model
    def _get_scheduled_hours_for_day(self, contract, work_date):
        """Use the effective roster shift, falling back to the contract calendar."""
        employee = contract.employee_id
        if employee and 'hr.shift.roster.line' in self.env:
            roster_line = self.env['hr.shift.roster.line'].sudo().get_planned_line(
                employee, work_date, contract.company_id
            )
            if roster_line:
                start_time = roster_line.start_time
                end_time = roster_line.end_time
                shift_hours = end_time - start_time
                if shift_hours <= 0:
                    shift_hours += 24.0
                return max(0.0, shift_hours - roster_line.break_duration)

        calendar = contract.resource_calendar_id
        if not calendar:
            return 0.0
        timezone_name = calendar.tz
        tz = timezone(timezone_name)
        return calendar.get_work_hours_count(
            tz.localize(datetime.combine(work_date, time.min)),
            tz.localize(datetime.combine(work_date, time.max)),
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts.filtered(
            ...
            for day, hours, leave in day_leave_intervals:
                ...
                if len(leave) > 1:
                    ...
                    current_leave_struct = leaves.setdefault(
                        leave_type, {
                            'name': leave_type.name or _(
                                'Global Leaves'),
                            'sequence': 5,
                            'code': self._get_leave_type_code(leave_type),
                            'number_of_days': 0.0,
                            'number_of_hours': 0.0,
            ...
            attendance_records = attendance_model.search([
                ...
                # Use Odoo's employee-local attendance date. Regularization
                # writes the punches in UTC, so filtering raw datetimes can
                # exclude a valid corrected attendance near day boundaries.
                ('date', '>=', fields.Date.from_string(date_from)),
                ('date', '<=', fields.Date.from_string(date_to)),
                ('check_out', '!=', False),
            ])
            ...
            for item in c_leaves:
                if not leaves or item not in leaves:
                    data = {
                        'name': item.name,
                        'sequence': 20,
                        'code': self._get_leave_type_code(item, 'LEAVES'),
    ...
    @api.model
    def _get_leave_type_code(self, leave_type, fallback='GLOBAL'):
        """Use the configured code, including for legacy duplicate leave types."""
        leave_type = leave_type.sudo()
        code = (leave_type.code or '').strip()
        if code:
            return code
        if leave_type.name:
            leave_name = leave_type.name.strip().casefold()
            configured_types = self.env['hr.leave.type'].sudo().search([
                ('code', '!=', False),
            ], order='id')
            for configured_type in configured_types:
                if ((configured_type.name or '').strip().casefold()
                        == leave_name):
                    code = (configured_type.code or '').strip()
                    if code:
                        return code
        return fallback
    ...
    def _get_payslip_lines(self, contract_ids, payslip_id):
        ...
        # parent id as well
        contracts = self.env['hr.version'].browse(contract_ids)
        if len(contracts) == 1 and payslip.contract_id.struct_id:
            structure_ids = list(
                set(payslip.contract_id.struct_id._get_parent_structure().ids))
    ...
    def onchange_employee(self):
        ...
        if not self.env.context.get('contract') or not self.contract_id:
            ...
            if not self.contract_id.struct_id and not self.contract_id.contract_template_id.struct_id:
                return
            self.struct_id = self.contract_id.struct_id or self.contract_id.contract_template_id.struct_id
        if self.contract_id:
