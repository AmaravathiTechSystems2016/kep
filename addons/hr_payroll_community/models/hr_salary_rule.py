...
class HrSalaryRule(models.Model):
    ...
    def _compute_rule(self, localdict):
        """Evaluate the formula configured on this salary rule."""
        self.ensure_one()

        if 'worked_days' in localdict and hasattr(localdict['worked_days'], 'dict'):
            for code, worked_day_line in localdict['worked_days'].dict.items():
                localdict[code] = worked_day_line.number_of_days
        ...
        try:
            ...
            return (
                ...
                localdict.get('result_qty') or 1.0,
                localdict.get('result_rate') or 100.0,
            )
        except Exception:
            raise UserError(
                _('Wrong formula defined for salary rule %s (%s).') % (
                    self.name, self.code))
